...
def calculate_initial_threshold(cap, sample_size, base_threshold):
    ...
    for _ in range(sample_size - 1):
        ...
        if not ret:
            break
        total_movement += np.sum(cv2.absdiff(cv2.cvtColor(prev_frame, cv2.COLOR_BGR2GRAY), cv2.cvtColor(current_frame, cv2.COLOR_BGR2GRAY)))
        prev_frame = current_frame

    average_movement = total_movement / ((cap.get(cv2.CAP_PROP_FRAME_HEIGHT) * cap.get(cv2.CAP_PROP_FRAME_WIDTH)) * (sample_size - 1))
...
def remove_dead_frames(video_path, base_threshold):
    ...
    movement_threshold = calculate_initial_threshold(cap, 30, base_threshold)  # Sample 30 frames for initial threshold
